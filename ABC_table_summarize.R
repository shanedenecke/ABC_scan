...
p = add_argument(p, "--indir",help='use domain filter key',default='/mnt/disk/shane/Transporter_ID/Arthropod_ABC_pipeline/ABC_search/TesSpe')
argv=parse_args(p)

## Put arguments into readable variables
minlen=argv$minlen
motif=argv$motif
...
################ Set defaults for motif and domain filter. 
if(!is.na(motif)){
  if(motif=="NA"){motif=NA}
}

if(!is.na(domain_filter)){
  if(domain_filter=="NA"){domain_filter=NA}
}
...
################ sort into categories based on filtering critera (domain, minlength, motifs)
filt.list=list()
good.list=list()
...
################ Solve ABC-B ambiguities
for(i in 1:nrow(good)){
  row=good[i,]
  if(row$fam=='ABCBAMG' & row$domains>=2){
    good[i]$fam='ABCBF'
    good[i]$name=gsub('ABCBAMG','ABCBF',good[i]$name)

  }else if (row$fam=='ABCBAMG' & row$domains<=1){
    good[i]$fam='ABCBH'
    good[i]$name=gsub('ABCBAMG','ABCBH',good[i]$name)
  }
}
...
################ Make fasta of each category (short, good, total)
